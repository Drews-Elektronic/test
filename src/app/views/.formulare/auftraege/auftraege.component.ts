...
export class AuftraegeComponent {
  ...
  GetAuftraege(aunr:number|string|undefined = undefined) {
    ...
    let subscription = this.backend.GetAuftraege(aunr, true).subscribe((value) => {
      subscription.unsubscribe();
  ...
  Preisermittlung(aunr: any, bgnr: any) {
    ...
    if (this.slide_toggle) {// Mit Quellen Aktualisieren und Leiterplatten Anfrage
      ...
      // let subscription0 = this.backend.QuellenAktualisieren("AUNR", 0, 0, 0, aunr, bgnr, 0, 0).subscribe((value) => {
      let subscription0 = this.backend.QuellenAktualisierenNew("AUNR", 0, aunr, 0, 0, 0, 0 /**<-Baugruppenmenge */).subscribe((value) => {
  ...
  Umleiten_nach_neuer_auftrag_baugruppen(){
    this.router.navigate(UtilUrl.neuesAngebot.neues_angebot_baugruppen)
  }

  Umleiten_nach_angebot_fortfahren(auftrag: any){
    this.router.navigate(UtilUrl.angebotFortfahren.angebot_fortfahren(auftrag.bgnr, auftrag.aunr))
  }
