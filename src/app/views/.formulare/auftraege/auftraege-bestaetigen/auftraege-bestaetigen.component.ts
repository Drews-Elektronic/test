...
export class AuftraegeBestaetigenComponent {
  ...
  angebot_nicht_loeschen: boolean = false;
  ...
  ngOnDestroy(){
    ...
    if(!this.angebot_nicht_loeschen && !this.auftrag_fortfahren){
  ...
  //#region Weiter
  angebot_bestaetigen(){
    if(this.auftraege[0].bestellt == 0){
      ...
      let subscription1 = dialogRefUebernahmeBeginnen.afterClosed().pipe(
        ...
        tap((result: any) => {                          // ja: true; nein: false;
          ...
          if(result){
            ...
            this.angebot_nicht_loeschen = true;
              ...
              .subscribe((value: any)=>{
                ...
                if(value !== false){
                  if(value?.checkout?.url){
                    //window.open(value?.checkout?.url);
  ...
  zurueck(){
    if(this.neues_projekt){
      ...
      this.router.navigate(UtilUrl.angebotFortfahren.zusammenfassung(this.bgnr, this.aunr))
    }else{
