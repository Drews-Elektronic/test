...
export class BaugruppenStuecklisteComponent implements OnInit {
    ...
    loescheStueckliste(
        ...
        if(stueckliste?.selected && Array.isArray(stueckliste?.selected)){
            length = stueckliste.selected.length
            stueckliste_name = stueckliste.selected[0].btnrkd
        }else{
            length = 1
            ...
            if(this.selectedStueckliste.data.length > 0){
                ...
                let subscription = this.backend.verfuegbarkeitZuruecksetzen("bgnr", this.bgnr, true).subscribe(async (value) => {
                    ...
                    if(value !== false){
                        ...
                        // Spinning für alle Bauteile beginnen
                        for (const element of this.KomprimiertStueckliste.data) {
                            element.spinning = true
                        }

                        this.falsches_laden = true

                        // Verfügbarkeits Prüfung beginnen
                        this.backend
                            .QuellenAktualisierenNew("BGNR", this.bgnr, this.baugruppenmenge, 0, "", false)
                            .pipe(take(1))
                            .subscribe((value: any) => {
                                ...
                                let subscription2 = this.backend
                                    ...
                                    .subscribe((value: any) => {
                                        ...
                                        if(value !== false){
                                            ...
                                            this.KomprimiertStueckliste.data = value
                                        }
    ...
    async alle_verfuegbarkeiten_pruefen_in_10er_schritte(){
        return new Promise((resolve, reject)=>{
    ...
    private async verfuegbarkeit_pruefen(element: any, ladebalken_verwenden:boolean = false, message: boolean = true){
        return new Promise((resolve, reject) => {
            element.spinning = true;
            ...
            if(heute_geprueft){
                ...
                this.backend
                    .QuellenAktualisierenNew("SLNR", element.slnr, this.baugruppenmenge, 1, element.btnrkd, message)
                    .pipe(take(1))
                    .subscribe((value: any) => {

                        // Stücklisten/BOM Zeile soll neugeladen werden, egal ob "value" false oder was anderes ist. 
                        let subscription2 = this.backend
                            .GetStueckliste(element.bgnr, element.slnr)
                            .subscribe((value: any) => {
                                subscription2.unsubscribe();
